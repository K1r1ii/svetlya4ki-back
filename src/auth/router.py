...
async def create_invite(email: str, user: User = Depends(get_current_user_by_access_token)):
    ...
    invitation: Invitation = InvitationDAO.add_one((str(uuid.uuid4()), str(user.company_id), email, token))

    await RegisterService.send_invite_email(email, token)
...
async def refresh_jwt(user: User = Depends(get_current_user_by_refresh_token)):
    ...
    data_for_token = {"user_id": str(user.id)}
